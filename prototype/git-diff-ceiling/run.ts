...
for (const kind of ["broad","threshold"] as const) {
  const cwd = await fixture(kind);
  for (const lens of ["standards","spec"] as const) for (const repetition of [1,2]) for (const ceiling of [50,80]) {
    ...
    await writeFile(join(out,`${name}.jsonl`),result.stdout); await writeFile(join(out,`${name}.meta.json`),JSON.stringify({name,kind,lens,repetition,ceiling,latencyMs:result.ms,stderr:result.stderr},null,2));
  }
}
